...
class Data_Manager:
    ...
    def __init__(self, femgeomtery:FEMGeometry):
        ...
        # coarse-scale fields during an iteration
        self.u_c_it = np.zeros((self.femgeomtery.dof_per_node*self.femgeomtery.mesh.nodes.shape[0],1))
        self.v_c_it = np.zeros((self.femgeomtery.dof_per_node*self.femgeomtery.mesh.nodes.shape[0],1))
        ...
        # fine-scale fields during an iteration
        self.u_f_it = np.zeros(self.femgeomtery.mesh.nodes_m.shape[0],self.femgeomtery.dof_per_node*self.femgeomtery.mesh.nodes_m.shape[1])
        self.v_f_it = np.zeros(self.femgeomtery.mesh.nodes_m.shape[0],self.femgeomtery.dof_per_node*self.femgeomtery.mesh.nodes_m.shape[1])
    ...
    def update_coarse_acceleration_substep(self):
        ...
        for unit_cell_no in range(m):
            for macro_el_no in range(n):
                ...
                # coarse-scale entities per coarse-element 
                u_c_e = self.data.u_c[self.femgeomtery.dof_conn_coarse[ie_macro,:]]
                X_c_e = self.femgeomtery.mesh.nodes[self.femgeomtery.mesh.elements[ie_macro,:],:]

                # fine-scale entities per coarse-element
                u_f = self.data.u_f[unit_cell_no,:]
                a_f = self.data.a_f[unit_cell_no,:]
                X_f = self.femgeomtery.mesh.nodes_m[unit_cell_no]
    ...
    def update_coarse_acceleration_fullstep(self):
        ...
        for unit_cell_no in range(m):
            for macro_el_no in range(n):
                ...
                a_f = self.data.a_f_sub[unit_cell_no,:]
                X_f = self.femgeomtery.mesh.nodes_m[unit_cell_no]
    ...
    def update_fine_acceleration_substep(self):
        ...
        for unit_cell_no in range(self.femgeomtery.mesh.nel_patch):
            ...
            for macro_elem_no in range(macro_elms):
                ...
                u_c_e = self.data.u_c[self.femgeomtery.dof_conn_coarse[self.femgeomtery.mesh.el_list_patch[unit_cell_no,macro_elem_no],:]]
                a_c_e = self.data.a_c[self.femgeomtery.dof_conn_coarse[self.femgeomtery.mesh.el_list_patch[unit_cell_no,macro_elem_no],:]]
            ...
            # update the iterate value
            self.data.a_c_it[unit_cell_no,:]  = np.divide(np.multiply(-1.0,Fg_int), self.Mg)
    ...
    def update_fine_acceleration_fullstep(self):
        ...
        for unit_cell_no in range(self.femgeomtery.mesh.nel_patch):
            ...
            for macro_elem_no in range(macro_elms):
                ...
                a_c_e = self.data.a_c_sub[self.femgeomtery.dof_conn_coarse[self.femgeomtery.mesh.el_list_patch[unit_cell_no,macro_elem_no],:]]
                X_c_e = self.femgeomtery.mesh.nodes[self.femgeomtery.mesh.elements[self.femgeomtery.mesh.el_list_patch[unit_cell_no,macro_elem_no],:],:]
